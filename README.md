...
GROQ_DOCUMENT_MODEL=qwen/qwen3.8-27b
GROQ_MODEL=openai/gpt-oss-120b
```

The diagnoser, remediation generator, explainer, roadmap, and uploaded-work document-analysis endpoints are routed independently, so their requests can use separate Groq rate limits concurrently. The document agent analyzes uploaded images with Groq vision and extracts text from PDF and DOCX files before it creates the first targeted practice question.
